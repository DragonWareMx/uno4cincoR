...
@endif
    ...
    <div class="div_containerAuthor">
        <div class="div_datosAuthor">
            <form action="{{ route('editarColeccion',['id'=>$coleccion->id]) }}" style="width:100%;" method="POST" enctype="multipart/form-data">
                @method("PATCH")
                @csrf
                <div class="div_elementosAuthor">
                    <p class="txt_datosAuthor">Nombre de colección:</p>
                    <input name="nombre" class="input_datosAuthor" type="text" value="{{$coleccion->nombre}}" required autofocus>
                </div>

                <div class="div_elementosAuthor" style="margin-bottom: 25px">
                    <p class="txt_datosAuthor">Descripción:</p>
                    <textarea class="textarea_biografia"type="text" name="descripcion" value="" required >{{$coleccion->descripcion}}</textarea>
                </div>
